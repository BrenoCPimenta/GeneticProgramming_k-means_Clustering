...
class SubjectTree():
    ...
    def getDeph(self):
        return self.root.getNodeDeph()

    def mutate(self):
        deph = self.root.getNodeDeph()
        mutation_deph = np.random.randint(deph+1)
        #mutation_subsequent_tree_deph = (deph - mutation_deph)
        self.root.mutate(mutation_deph)
...
class Node(SubjectTree):
    ...
    def getNodeDeph(self):
        return self.num_layer
    
    def mutate(self, deph):
        if deph != 0: #keep digging
            side_decision = np.random.choice(['left', 'right'])
            if side_decision == 'left':
                self.left_node.mutate(deph - 1)
            elif side_decision == 'right':
                self.right_node.mutate(deph - 1)
            else:
